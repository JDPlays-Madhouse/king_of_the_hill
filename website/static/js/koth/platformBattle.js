...
export class Scoreboard {
    ...
    startChatMessage(platform, lastWinner = undefined) {
        const score = this.scoreChatMessage;
        const diff = this.diff;
        const state = this.overallScoreState(platform);
        console.log({ state, platform });
        const lastWinnerMessage = lastWinner === undefined ? "" : ` and the defending ${lastWinner.joinTitle} is ${lastWinner.username}`;

        switch (state) {
            case PlatformScoreState.WINNING:
                return `You are currently winning ${platform}${lastWinnerMessage}. The score is ${score}, keep up the great work!`;
            case PlatformScoreState.LOSING:
                return `You are currently losing ${platform}${lastWinnerMessage}. The score is ${score}, you are only behind by ${diff}! Remember you can kick them in the shins if you need to!!`;
            case PlatformScoreState.TYING:
                return `We are all tied up ${platform}${lastWinnerMessage}. The score is ${score}, we have this, push ahead so we can be the champions.`;
            default:
