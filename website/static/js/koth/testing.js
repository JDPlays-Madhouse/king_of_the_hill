import { weaponObjects, weaponNames, usersWeapon as uW } from "./weapons.js";
...
function addFighter(user, weapon, imageUrl, platform, side) {
	...
	xhttp.onreadystatechange = function () {
		if (this.readyState === 4 && this.status === 200) {
			const warp = document.getElementById("confetti-container");
...
function weaponTest(annimationSide = null, inputWeapon = weaponName) {
	...
	if (weaponNames.includes(inputWeapon)) {
		...
		weapon = uW(inputWeapon);
	}
	const imageUrl = "https://decapi.me/twitch/avatar/" + testingUser;
...
function regexWeaponTesting() {
	const container = document.getElementById("weaponRegexTesting");
	const inputBox = document.createElement("input");
	inputBox.id = "weaponRegexTestingInput";
	inputBox.setAttribute("autofocus", true);
	inputBox.placeholder = "mega choo choo";
	inputBox.title = "Test Weapon Names";
	inputBox.addEventListener("keyup", function onEvent(e) {
		const input = e.target.value;
		console.log(input.length);
		if (input.length === 0) {
			document.getElementById("weaponRegexTestingDisplay").innerText = "";
			return;
		}
		const weapon = uW(input);
		document.getElementById("weaponRegexTestingDisplay").innerText =
			weapon.name;
	});
	...
	display.id = "weaponRegexTestingDisplay";
...
function sidesUserWinner(sideButtonDiv) {
	...
	btn.onclick = () => {
		...
		const btn = document.getElementById("riggedWinner");
...
function setPlatformSides() {
	const ytBorder = "var(--yt-primary) solid";
	const twitchBorder = "var(--twitch-primary) solid";
	const left = PlatformSide.Twitch === SIDE.left ? twitchBorder : ytBorder;
	const right = PlatformSide.Twitch === SIDE.right ? twitchBorder : ytBorder;
